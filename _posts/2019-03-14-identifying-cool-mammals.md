...
We still have two steps ahead: 1) convert the `XML` annotations to a `CSV` file and 2) convert the `CSV` files to Tensorflow `RECORDS` files. A nice way to organize the folders and scripts can be found [here](https://pythonprogramming.net/creating-tfrecord-files-tensorflow-object-detection-api-tutorial/).
